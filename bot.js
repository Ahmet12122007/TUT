...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Streetlive Roleplay Server Status", "https://cdn.discordapp.com/attachments/580680574369398794/582249284825776148/logo_vinity_end.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Streetlive Roleplay' : '⚫ Streetlive Roleplay')
    .setTimestamp(new Date())
    .addField('\n\u200b\nWie kann man dem Server joinen?','Jeder kann dem Server joinen mit **162.19.161.32:30044** mit Direct Join.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .addField('Warteschlange','?',true)
    .addField('Online Spieler','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Warteschlange',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Online Spieler',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('Streetlive Roleplay',{'url':'https://www.twitch.tv/kampfkektv','type':'STREAMING'});
